...
namespace lightwave
{
    ...
    {
        lightwave_throw("cannot convert string into %s", demangle(typeid(To).name()));
    }
    ...
    class Properties
    {
        using Value = std::variant<float, int, bool, std::string, Color, Vector, ref<Object>>;

        static std::string toString(const Value &value)
        {
            return std::visit(overloaded{[&](auto &&arg) { return tfm::format("%s", arg); },
                                         [&](const std::string &str) { return tfm::format("\"%s\"", str); },
      ...
      public:
        ...
        template <typename T>
        std::enable_if_t<!std::is_base_of_v<Object, T>, void> set(const std::string &name, const T &value)
        {
            if (has(name))
            {
        ...
        /// @brief Sets an attribute to the given value.
        template <typename T>
        std::enable_if_t<std::is_base_of_v<Object, T>, void> set(const std::string &name, const ref<T> &object)
        {
            if (has(name))
            {
                lightwave_throw("property \"%s\" redefined", name);
        ...
        template <typename T>
        std::enable_if_t<!std::is_base_of_v<Object, T> && !std::is_same_v<T, std::string>, T> get(
            const std::string &name) const
        {
            ...
            return std::visit(overloaded{[&](auto &&arg) -> T {
                                             lightwave_throw("cannot cast %s into %s", demangle(typeid(arg).name()),
        ...
        /// @brief Gets the value of an attribute.
        template <typename T> std::enable_if_t<std::is_same_v<T, std::string>, T> get(const std::string &name) const
        {
            auto v = m_attributes.find(name);
            if (v == m_attributes.end())
            {
            ...
            m_unqueriedAttributes.erase(name);
            return std::visit(overloaded{[&](auto &&arg) -> std::string {
                                             lightwave_throw("cannot cast %s into %s", demangle(typeid(arg).name()),
        ...
        template <typename T>
        std::enable_if_t<!std::is_base_of_v<Object, T>, T> get(const std::string &name, const T &fallback) const
        {
            auto v = m_attributes.find(name);
            if (v == m_attributes.end())
            {
        ...
        /// @brief Gets the value of an attribute.
        template <typename T> std::enable_if_t<std::is_base_of_v<Object, T>, ref<T>> get(const std::string &name) const
        {
            auto v = m_attributes.find(name);
            if (v == m_attributes.end())
            {
        ...
        /// attribute is not present.
        template <typename T>
        std::enable_if_t<std::is_base_of_v<Object, T>, ref<T>> get(const std::string &name,
                                                                   const ref<T> &fallback) const
        {
            auto v = m_attributes.find(name);
            if (v == m_attributes.end())
            {
        ...
        /// matching option.
        template <typename T>
        T getEnum(const std::string &name, const std::vector<std::pair<std::string, T>> &options) const
        {
        ...
        template <typename T> ref<T> getChild(bool required = true) const
        {
            ...
            for (auto child : m_children)
            {
                if (auto casted = std::dynamic_pointer_cast<T>(child))
                {
                    if (result.get())
                    {
                        lightwave_throw("multiple %s children present", demangle(typeid(T).name()));
                    }
                    result = casted;
            ...
            {
                lightwave_throw("could not find required %s child", demangle(typeid(T).name()));
            }
            return result;
        }
        ...
        /// @brief Gets a child of a given type.
        std::vector<ref<Object>> children() const
        {
            return m_children;
        }
        ...
        ~Properties()
        {
            ...
            {
                logger(EWarn, "a child node was specified, but never queried: %s", child);
            }

            for (auto &property : m_unqueriedAttributes)
            {
                logger(EWarn, "attribute \"%s\" was specified, but never queried", property);
            }
        }
    };

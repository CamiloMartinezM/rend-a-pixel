    ...
    class Rectangle : public Shape
    {
      ...
      public:
        ...
        bool intersect(const Ray &ray, Intersection &its, Sampler &rng) const override
        {
            ...
            // (we ignore the edge case - pun intended - that the ray might have infinite intersections with the
            // rectangle)
            ...
            // we also do not update the intersection if a closer intersection already exists (i.e., its.t is lower than
            // our own t)
            ...
            // we have intersected an infinite plane at z=0; now dismiss anything outside of the [-1,-1,0]..[+1,+1,0]
            // domain.
        ...
        AreaSample sampleArea(Sampler &rng) const override
        {
            ...
            Point position{2 * rnd.x() - 1, 2 * rnd.y() - 1,
                           0}; // stretch the random point to [-1,-1]..[+1,+1] and set z=0

            AreaSample sample;
            populate(sample,
                     position); // compute the shading frame, texture coordinates and area pdf (same as intersection)
            return sample;
        }

...
namespace lightwave
{
    ...
    class TriangleMesh : public AccelerationStructure
    {
         ...
         * Note that multiple triangles can share vertices, hence there can also be fewer than @code 3 * numTriangles
         * @endcode vertices.
         */
      ...
      public:
        TriangleMesh(const Properties &properties)
        {
            ...
            logger(EInfo, "loaded ply with %d triangles, %d vertices", m_triangles.size(), m_vertices.size());
        ...
        std::string toString() const override
        {
            return tfm::format("Mesh[\n"
                               ...
                               "  filename = \"%s\"\n"
                               "]",
                               m_vertices.size(), m_triangles.size(), m_originalPath.generic_string());
        }
    };
} // namespace lightwave

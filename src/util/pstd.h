// Taken from: https://github.com/mmp/pbrt-v4/blob/master/src/pbrt/util/pstd.h
// Reason: Custom polymorphic allocator for DigitPermutation class in permutation_utils.h
...
namespace pstd
{
    ...
    template <typename T> class optional
    {
      public:
        ...
        T &value()
        {
            // CHECK(set); EDIT: Not using check.h
            return *ptr();
        }
    ...
    template <typename T> inline std::ostream &operator<<(std::ostream &os, const optional<T> &opt)
    {
        if (opt.has_value())
            return os << "[ pstd::optional<" << typeid(T).name() << "> set: true "
                      << "value: " << opt.value() << " ]";
        else
            return os << "[ pstd::optional<" << typeid(T).name() << "> set: false value: n/a ]";
    }
    ...
    namespace span_internal
    {
        ...
        template <typename C> inline constexpr auto GetDataImpl(C &c, char) noexcept -> decltype(c.data())
        {
            return c.data();
        }

        template <typename C> inline constexpr auto GetData(C &c) noexcept -> decltype(GetDataImpl(c, 0))
        {
        ...
        using HasData =
            std::is_convertible<typename std::decay_t<decltype(GetData(std::declval<C &>()))> *, T *const *>;
    ...
    template <typename T> class span
    {
      public:
        ...
        template <typename U> using EnableIfConstView = typename std::enable_if_t<std::is_const_v<T>, U>;

        // Used to SFINAE-enable a function when the slice elements are mutable.
        template <typename U> using EnableIfMutableView = typename std::enable_if_t<!std::is_const_v<T>, U>;
        ...
        T &operator[](size_t i)
        {
            // DCHECK_LT(i, size()); EDIT: Not using check.h
            return ptr[i];
        }
    ...
    template <int &...ExplicitArgumentBarrier, typename T> inline span<T> MakeSpan(T *begin, T *end) noexcept
    {
        return span<T>(begin, end - begin);
    }

    template <typename T> inline span<T> MakeSpan(std::vector<T> &v) noexcept
    {
    ...
    template <int &...ExplicitArgumentBarrier, typename T> inline span<const T> MakeConstSpan(T *begin, T *end) noexcept
    {
    ...
    monotonic_buffer_resource(void *buffer, size_t buffer_size)
        ...
        template <class Tp = std::byte> class polymorphic_allocator
        {
          public:
            using value_type = Tp;
    ...
    template <typename T, class Allocator = pmr::polymorphic_allocator<T>> class vector
    {
      public:
        ...
        template <class InputIt> vector(InputIt first, InputIt last, const Allocator &alloc = {}) : alloc(alloc)
        {
            reserve(last - first);
            size_t i = 0;
        ...
        {
            // LOG_FATAL("TODO"); EDIT: Not using log.h
            // TODO
        }
        ...
        {
            // DCHECK_LT(index, size()); EDIT: Not using check.h
            return ptr[index];
        }
        ...
        void pop_back()
        {
            // DCHECK(!empty()); EDIT: Not using check.h
        ...
        {
            // CHECK(alloc == other.alloc); // TODO: handle this EDIT: Not using check.h
    ...
    template <size_t I, typename T, typename... Ts> auto &get(tuple<T, Ts...> &t)
    {
        if constexpr (I == 0)
            return t.value;
        else
    ...
    template <size_t I, typename T, typename... Ts> const auto &get(const tuple<T, Ts...> &t)
    {
        if constexpr (I == 0)
            return t.value;
        else
    ...
    template <typename Req, typename T, typename... Ts> auto &get(tuple<T, Ts...> &t)
    {
    ...
    template <typename Req, typename T, typename... Ts> const auto &get(const tuple<T, Ts...> &t)
    {
        if constexpr (std::is_same_v<Req, T>)
            return t.value;
        else
